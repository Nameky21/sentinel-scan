...
**Fastest path: the "SentinelScan" desktop shortcut** (created once via `.\scripts\install-shortcuts.ps1`). It opens `scripts/SentinelScanApp.ps1`, a small WinForms control panel with one button that starts ZAP + backend + frontend and swaps to a Stop button once `/api/zap/status` confirms the *backend* is actually connected to ZAP (not just that ZAP itself is up). Under the hood it's just `start-all.ps1` / `stop-all.ps1 -Quiet`, run hidden with output logged to `scripts/.run/logs/`.

Run `.\scripts\start-all.ps1` / `.\scripts\stop-all.ps1` directly (no `-Quiet`) instead of through the GUI when you want to see live console progress. Both are idempotent — safe to re-run, they skip anything already listening on its port.

### PowerShell gotchas on this machine (learned the hard way — don't relearn them)

- **`Start-Job` does not survive between separate tool calls.** Each PowerShell/Bash tool invocation is effectively a fresh process; shell state (jobs, variables) doesn't persist across calls. Anything that must outlive the current command needs `Start-Process` (a real detached OS process), never `Start-Job`.
- **`Start-Process -Wait` blocks on the whole process tree, not just the direct child.** `start-zap.ps1` intentionally leaves ZAP's `java.exe` running after it exits — waiting on it with `-Wait` hangs forever. Use `$proc = Start-Process -PassThru; $proc.WaitForExit()` (waits only for that specific process). `.ExitCode` reads as `$null` until the process has actually exited — check `.HasExited` (or that `WaitForExit()` returned) before trusting it, and prefer a real side effect (e.g. the expected port is listening) as the success signal when you can.
- **`uvicorn --reload` and `npm run dev` (vite) each spawn child processes.** Killing only the top-level PID leaves the real listener orphaned. Use `taskkill /F /T /PID <pid>` (tree-kill), not `Stop-Process`.
- **Never `taskkill` a PID read from a state file without validating it first.** Windows recycles PIDs; a PID saved earlier can later belong to an unrelated (possibly protected) process, and blindly killing it can fail with "Access is denied" even elevated. Check the process name (and ideally path/command-line) still matches before killing — see `stop-all.ps1`'s `$expected` table. Two specifics that tripped this up: a `java.exe` process's `.Path` is the JRE install location, never the jar's — verify via `(Get-CimInstance Win32_Process -Filter "ProcessId=$pid").CommandLine` instead; and `Start-Process` on a `.cmd` file (e.g. `npm.cmd`) returns `cmd.exe`'s PID, not `node.exe`'s.
